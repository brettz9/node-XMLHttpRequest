/* eslint-disable unicorn/no-this-assignment -- Convenient */
'use strict';
...
function parseUrl (urlString) {
  try {
    ...
    return {protocol, hostname, port, pathname, search, path: pathname + search};
  } catch {
    const [pathname, search = ''] = urlString.split(/(?=\?)/);
    return {protocol: undefined, hostname: undefined, port: undefined, pathname, search, path: pathname + search};
  }
}
 ...
 * }} [cfg]
 */
module.exports = function localXMLHttpRequest (cfg) {
  cfg = cfg || {};
  ...
  XMLHttpRequest.prototype.send = function (data) {
    ...
    case '': {
      ...
      const basePath = cfg.basePath || path.dirname(getStack().reverse().find(function (item) {
        const filename = item.getFileName();
        const idx = filename.search(/[/\\]node_modules[/\\]/);
        ...
        // var pathName = new URL(this._settings.url, basePath).pathname; // We should support this instead if the config were relative to URL
    ...
    if (blobURL) {
      if (this._settings.method !== 'GET') {
        throw new Error('XMLHttpRequest: Only GET method is supported for blob: URLs');
      }
      ...
      function respondWithBlob () {
        const resolved = cfg.resolveBlobURL && cfg.resolveBlobURL(self._settings.url);
        if (!resolved) {
          self.handleError(new Error('Failed to load ' + self._settings.url));
          return;
        }
        self._response = {
        ...
        self.status = 200;
        self.responseText = Buffer.isBuffer(resolved.bytes)
      ...
      if (this._settings.async) {
        process.nextTick(respondWithBlob);
      } else {
        respondWithBlob();
      }
    ...
    if (this._settings.user) {
      ...
      const authBuf = Buffer.from(this._settings.user + ':' + this._settings.password);
    ...
    function responseHandler (resp) {
      ...
      if (self._response.statusCode === 301 || self._response.statusCode === 302 || self._response.statusCode === 303 || self._response.statusCode === 307) {
        // Change URL to the redirect location
        self._settings.url = self._response.headers.location;
        url = parseUrl(self._settings.url);
        ...
        const newOptions = {
          ...
          port: url.port,
          path: url.path,
          method: self._response.statusCode === 303 ? 'GET' : self._settings.method,
          headers: self._headers
        };

        // Issue the new request
        self._request = doRequest(newOptions, responseHandler).on('error', errorHandler);
        self._request.end();
      ...
      self._setState(self.HEADERS_RECEIVED);
      self.status = self._response.statusCode;
    ...
    if (this._settings.async) {
      ...
      const execString = "var http = require('node:http'), https = require('node:https'), fs = require('node:fs');" +
        ...
        "fs.unlinkSync('" + syncFile + "');" +
        '});' +
        (data ? "req.write('" + JSON.stringify(data).slice(1, -1).replaceAll('\'', String.raw`\'`) + "');" : '') +
        'req.end();';
      ...
      if ((self.responseText).startsWith('NODE-XMLHTTPREQUEST-ERROR:')) {
        // If the file returned an error, handle it
        const errorObj = self.responseText.replace(/^NODE-XMLHTTPREQUEST-ERROR:/, '');
        self.handleError(errorObj);
      } else {
        // If the file returned okay, parse its data and move to the DONE state
        self.status = parseInt(self.responseText.replace(/^NODE-XMLHTTPREQUEST-STATUS:(\d*),.*/, '$1'));
        self.responseText = self.responseText.replace(/^NODE-XMLHTTPREQUEST-STATUS:\d*,(.*)/, '$1');
        self._setState(self.DONE);
      }
    }
  };
