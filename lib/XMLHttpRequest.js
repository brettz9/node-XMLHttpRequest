...
module.exports = function localXMLHttpRequest (cfg) {
  ...
  class XMLHttpRequest {
    ...
    _isAllowedHttpHeader (header) {
      return this._disableHeaderCheck || Boolean(header && !forbiddenRequestHeaders.includes(header.toLowerCase()));
    }
    ...
    send (data) {
      ...
      /**
       * @type {Partial<import('url').UrlWithStringQuery> & {
       *   pathname: string|null
       * }}
       */
      let url = Url.parse(this._settings.url ?? '');
      let host;
      ...
      function getStack () {
        ...
        const stack = /** @type {unknown} */ (err.stack);
        Error.prepareStackTrace = orig;
        return /** @type {NodeJS.CallSite[]} */ (stack);
      }
      ...
      case '': {
        ...
        // eslint-disable-next-line unicorn/no-array-reverse -- Not yet supported
        const basePath = cfg?.basePath || path.dirname(stack?.reverse().find(function (item) {
          ...
          // Should be a user file because its last "node_modules" contains
          //  this XMLHttpRequest file (i.e., XMLHttpRequest is a dependency
          //  of some kind)
      ...
      this._headers.Host = /** @type {keyof http.OutgoingHttpHeaders} */ (
        host
      );
      ...
      function responseHandler (resp) {
        ...
        self.status = this._response.statusCode ?? null;
      ...
      if (this._settings.async) {
        ...
        if ((self.responseText).startsWith('NODE-XMLHTTPREQUEST-ERROR:')) {
          ...
          self.handleError(new Error(errorObj));
        } else {
    ...
    dispatchEvent (event) {
      ...
      case 'readystatechange':
        ...
        break;
      case 'load':
        if ('load' in self && typeof self.load === 'function') {
          self.load();
        }
        break;
      case 'loadend':
        if ('loadend' in self && typeof self.loadend === 'function') {
          self.loadend();
        }
        break;
      case 'loadstart':
        if ('loadstart' in self && typeof self.loadstart === 'function') {
          self.loadstart();
        }
        break;
      default:
        if (event in self && typeof self[event] === 'function') {
          // @ts-expect-error Ok
          self[event]();
        }
        break;
